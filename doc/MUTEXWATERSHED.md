...
(`python doc/assets/mutex_watershed_slide.py`); edit the `PULL` / `PUSH` /
...
cached viz batch (3-D) and logs the segmentation panels.  The visualisation
gates the agglomeration on predicted `sem > sem_gate_threshold` (default
`0.5`) only when `training.mutex_watershed.gate_with_sem` is true (the
default; set in the flagship configs); when false, MWS runs unmasked.  (The
eval instance metric in §5.1 gates with the GT foreground regardless of this
setting.)
...
{stage}/automatic/true/wan_decoder  # Cosmos + VAE only (Wan-decoder recon)
{stage}/automatic/aff/pred/{offset} # all N_AFF affinity channels
{stage}/automatic/aff/true/{offset} # GT affinity (3-D)
```

The `true/wan_decoder` panel is emitted only for Cosmos backbones with a VAE
decoder (the Wan-decoder reconstruction), i.e. when a `wan_decoder_2d` tensor
is passed to `_log_predictions`; it is absent for other backbones.
...
On CUDA inputs (the norm on a GPU trainer) the default agglomeration is the
GPU torch Boruvka (`mws_th`), run entirely on-device (`backend: auto` ⇒
`torch` for any CUDA tensor); the sequential numba union-find core
(`mws_np` / `_mws_core`) only runs on CPU-tensor inputs, when `backend: cpu`
is forced, or as the exact reference in tests.  Either way the instance
metrics run MWS on every validation crop, so it is the heaviest part of
